...
def encrypt(seed):
    ''' Encrypts the message and outputs the encrypted message and decryption key
    INPUTS: user, seed (integer)
    OUTPUTS: enc_message (string), enc_hash (string) '''
    ...
    # Produce a secure random alphanumeric encryption hash string from ascii
    ...
    # Calculate the value lists for the message and the hash
    message_values = charstring_to_valuelist(message, seed)
    key_values = charstring_to_valuelist(key, seed)
    
    # Operate between the message and the hash values using XOR
    ...
    # Return the string of the encrypted message and the string of the hash
    return enc_message, key

#-------------------------------------------------------------------------------

def decrypt(seed):
    ''' Decrypts the encrypted message, requires the corresponding decryption key
    INPUTS: user, seed (integer)
    ...
    while True:
        ...
        try:
            ...
            # Ask the user to input the string of the hash
            key = input("Enter the unique alphanumeric key:\nINPUT:")
            
            # Check that the encryption hash does not contain symbols or spaces
            if key.isalnum() != True:
                print("\n***Invalid input - Hash contains non-numeric and/or non-alphabetic characters***")
                # Raise an error
                raise Exception
            ...
            # Check that the message is the same length as the encryption hash
            if len(enc_message_value_blocks) != len(key):
                print("\n***Invalid input - Hash and message are not the same length***")
                # Raise an error
                raise Exception
    ...
    # Calculate the value list of the encryption hash
    key_values = charstring_to_valuelist(key, seed)
...
def HASHcrypt():
    ...
    if selection == 1:
        enc_message, enc_hash = encrypt(input_seed)
